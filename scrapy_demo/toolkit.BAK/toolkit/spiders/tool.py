...
class ToolSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for tool in tools:
            ...
            name_cn = tool.xpath('./td/div[@class="spte-tool-list-desc breakword" style="width:680px;padding-left:0px"]/text()').extract()[0].strip()

            size = tool.xpath('./td[@width="11%" style="text-align:center;"/text()')[0]

            release_date = tool.xpath('./td[@width="12%" style="text-align:center;"]/text()').extract()[0]

            download_cnt = tool.xpath('./td[@width="10%" style="text-align:center;"]/text()').extract()[0]
